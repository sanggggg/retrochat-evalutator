...
class Trainer:
    ...
    def save_rubrics(
        ...
        if (
            ...
            try:
                self._save_clustering_visualization(
                    result_folder,
    ...
    def _save_clustering_visualization(
        self,
        result_folder: Path,
        embeddings: np.ndarray,
        clusters: np.ndarray,
        all_rubrics: list[Rubric],
        cluster_info: Optional[dict[int, dict]],
    ) -> None:
        ...
        for i, cluster_id in enumerate(unique_clusters):
            ...
            cluster_rubrics = [all_rubrics[j] for j in range(len(all_rubrics)) if mask[j]]
