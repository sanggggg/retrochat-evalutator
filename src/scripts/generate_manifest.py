...
def process_jsonl_file(jsonl_path: Path, input_dir: Path, min_size_kb: int = 4) -> Optional[Dict[str, Any]]:
...
def generate_manifest(input_dir: str, output_file: str, min_size_kb: int = 4):
    ...
    for jsonl_file in sorted(jsonl_files):
        ...
        if (processed + skipped) % 100 == 0:
            print(f"  Processed: {processed + skipped}/{len(jsonl_files)} (kept: {processed}, skipped: {skipped})")
    ...
    if sessions:
        ...
        total_duration = sum(s['metadata'].get('session_duration_minutes', 0) or 0 for s in sessions)
        total_growth = sum(s['metadata'].get('net_growth', 0) for s in sessions)
        
        print(f"\nSummary:")
        print(f"  Total tokens: {total_tokens:,}")
        print(f"  Total session duration: {total_duration:.1f} minutes ({total_duration/60:.1f} hours)")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help='Minimum file size in KB to include (default: 4)'
    )
